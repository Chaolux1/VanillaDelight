...
public class CabinetQuadBuilder {
    ...
    public static BakedQuad create(Direction direction, float minA, float minB, float maxA, float maxB, float offset, TextureAtlasSprite textureAtlasSprite,int tint,float shade,boolean stretch,boolean emissive) {
        float a0=minA / 16.0f;
        float a1=maxA / 16.0f;
        float b0=minB / 16.0f;
        float b1=maxB / 16.0f;
        ...
        return createNormalizedRegion(direction,minA,minB,maxA,maxB,offset,textureAtlasSprite,tint,shade,normalizeU0,normalizeV0,normalizeU1,normalizeV1,emissive);
    }

    public static BakedQuad createNormalizedRegion(Direction direction,float minA,float minB,float maxA,float maxB,float offset,TextureAtlasSprite textureAtlasSprite,int tint,float shade,float normalizeU0,float normalizeV0,float normalizeU1,float normalizeV1,boolean emissive) {
        float a0=minA / 16.0f;
        float a1=maxA / 16.0f;
        float b0=minB / 16.0f;
        float b1=maxB / 16.0f;
        ...
        return createWithUV(direction,a0,b0,a1,b1,offset,textureAtlasSprite,tint,shade,u0,v0,u1,v1,emissive);
    }
    ...
    public static BakedQuad create(Direction direction,float minA,float minB,float maxA,float maxB,float offset,TextureAtlasSprite textureAtlasSprite,int tint,float shade,boolean stretch) {
        return create(direction,minA,minB,maxA,maxB,offset,textureAtlasSprite,tint,shade,stretch,false);
    }
    ...
    private static void putVertex(int[] intData,int vertices,Vertex vertex,int color,int light,int normal) {
    ...
    public static BakedQuad createRegion(Direction direction, float minA, float minB, float maxA, float maxB, float offset, TextureFillPattern textureFillPattern,float sourceOffsetX,float sourceOffsetY,float width,float height,int tint,float shade) {
        ...
        return createNormalizedRegion(direction,minA,minB,maxA,maxB,offset,textureAtlasSprite,tint,shade,normalizedU0,normalizedV0,normalizedU1,normalizedV1,false);
    }

    private static BakedQuad createWithUV(Direction direction,float a0,float b0,float a1,float b1,float offset,TextureAtlasSprite textureAtlasSprite,int tint,float shade,float u0,float v0,float u1,float v1,boolean emissive) {
        ...
        int light=emissive ? LightTexture.FULL_BRIGHT : 0;
        for(int index=0;index < vertices.length;index++) {
            putVertex(intData,index,vertices[index],color,light,normal);
        }
        return new BakedQuad(intData,-1,direction,textureAtlasSprite,!emissive,!emissive);
    }
